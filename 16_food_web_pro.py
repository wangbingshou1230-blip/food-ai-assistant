...
# ==================================================
#  RAG 2.0 向量库连接 (Pinecone) - 【修改点2：核心修复】
# ==================================================
@st.cache_resource
def get_vector_store():
    if "PINECONE_API_KEY" not in st.secrets:
        return None
    try:
        # 【修复】：使用 3.0+ 新版初始化方式，解决 "pinecone-client renamed" 报错
        ...
        # 【修复】：传递 index 对象而非 index_name 字符串
...
st.sidebar.caption("v12.0 | 2026 时代见证版") 

# 【修改点3】：新增侧边栏风险预警
st.sidebar.markdown("---")
st.sidebar.subheader("🛡️ 2026 风险中心")
st.sidebar.error("⚠️ 极高风险 (01-24)")
st.sidebar.caption("张又侠、刘振立立案审查。装备采购链面临深层审计。")
...
if app_mode == "🔬 R&D 研发中心":
    ...
    with tabs[3]:
        ...
        if not vector_store:
            st.warning("⚠️ 未检测到 PINECONE_API_KEY 或连接失败")
        else:
...
# --------------------------------------------------
#  MODE 3: 时代见证(投资避险) - 【修改点5：全新增模块】
# --------------------------------------------------
elif app_mode == "📈 时代见证(投资避险)":
    st.title("📈 时代见证：2026 资产避险看板")
    st.info("今日信号（2026-01-24）：中央军委副主席张又侠、联合参谋部参谋长刘振立被立案审查。")

    c1, c2 = st.columns(2)
    with c1:
        st.subheader("⚠️ 重点出清领域")
        st.markdown("""
        - **军工采购链**：装备部及下属供应链将面临深度审计，建议规避相关ETF及个股。
        - **传统实权派关联企业**：规避与旧势力有深厚背景关联的私企。
        """)
    with c2:
        st.subheader("🛡️ 安全防御方向")
        st.success("向‘大一统’保供体系靠拢")
        st.markdown("""
        1. **战略物资/粮食**：国家保供单位（利好你的食品专业领域）。
        2. **国产自研硬科技**：纯粹的技术导向型国之重器。
        3. **流动性**：建议提升现金或黄金占比。
        """)

    st.divider()
    st.subheader("🤖 AI 风险研判")
    query = st.text_input("输入股票或行业（如：军工ETF、航天发展），DeepSeek R1 深度研判风险：")
    if query:
        with st.spinner("R1 正在进行逻辑压力测试..."):
            sys_p = "你是一名顶级政经分析师。2026年1月24日张又侠案官宣，标志着军方装备采购系统面临大清洗。请基于此背景，分析用户输入的行业或股票的投资冲击。"
            r, a = call_deepseek_advanced([{"role":"system","content":sys_p},{"role":"user","content":query}], "reasoner")
            if r: st.expander("🧠 深度推理过程 (CoT)").markdown(r)
            st.markdown(a)

# --------------------------------------------------
#  MODE 4: 云端档案库 (完全保留原代码)
# --------------------------------------------------
